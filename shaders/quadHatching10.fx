...
float4 testFuncFrag(vertexOutput i) : SV_Target {
    int3 loc = int3(i.pos.xy, 0); // coordinates for loading

    // get pixel values
    float3 substrateNormal = gSubstrateTex.Load(loc).rgb;
    substrateNormal = 2.0 * substrateNormal - 1.0;

    return float4(substrateNormal, 1.0);
}
...
technique11 testTechnique {
	pass p0 {
		SetVertexShader(CompileShader(vs_5_0, quadVert()));
		SetGeometryShader(NULL);
		SetPixelShader(CompileShader(ps_5_0, testFuncFrag()));
	}
};